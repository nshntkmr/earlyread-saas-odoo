...
function getSteps(chartType, scopeMode, masterRowConfig = null) {
  ...
  if (chartType === 'record_header') {
    // SQL-only identity header — no Widget Controls / Click Actions. Reuses the
    // Columns step to map title (x_column) + footer value columns (y_columns);
    // avatar defaults to initials. LivePreview renders the header.
    return [
      { key: 'chart_type', label: 'Chart Type' },
