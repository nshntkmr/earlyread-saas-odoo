...
export default function AppPagePicker({ definition, apiBase, initialApps, onDone, onBack }) {
  ...
  const [selectedRegion, setSelectedRegion] = useState('tab_content')
    ...
    try {
      await designerFetch(libraryPlaceUrl(apiBase, definition.id), {
        ...
        body: JSON.stringify({
          ...
          render_region: selectedRegion,
          // page_summary is tab-independent → never send a tab.
          tab_id: selectedRegion === 'page_summary' ? null : (selectedTab?.id || null),
        }),
      })
      setSuccess(true)
    } catch (err) {
    ...
    <div className="dd-page">
      ...
      {apps.length === 0 ? (
        ...
        <div className="dd-place-tree">
          ...
          {/* Step 3: Placement Region */}
          {selectedPage && (
            <div className="dd-place-step">
              <h3 className="dd-place-step-title">
                <span className="dd-step-num">3</span> Placement Region
              </h3>
              <div className="dd-place-options">
                <button
                  type="button"
                  className={`dd-place-option ${selectedRegion === 'tab_content' ? 'dd-place-option--active' : ''}`}
                  onClick={() => setSelectedRegion('tab_content')}
                >
                  <i className="fa fa-window-maximize me-2" />
                  Tab Content (below tabs)
                </button>
                <button
                  type="button"
                  className={`dd-place-option ${selectedRegion === 'page_summary' ? 'dd-place-option--active' : ''}`}
                  onClick={() => setSelectedRegion('page_summary')}
                >
                  <i className="fa fa-thumb-tack me-2" />
                  Page Summary (above tabs — all tabs)
                </button>
              </div>
            </div>
          )}

          {/* Step 4: Select Tab (optional; not applicable for Page Summary) */}
          {selectedPage && selectedRegion !== 'page_summary' && selectedPage.tabs && selectedPage.tabs.length > 0 && (
            <div className="dd-place-step">
              <h3 className="dd-place-step-title">
                <span className="dd-step-num">4</span> Select Tab <span className="text-muted">(optional)</span>
              </h3>
              <div className="dd-place-options">
                <button
                  type="button"
