...
export function FilterProvider({ children, pageConfig, apiBase }) {
  ...
  // Live ref to the APPLIED values — immediate-apply derives its base from
  // here (never pending) so it can't commit another filter's unfinished edits.
  const filterValuesRef = useRef(filterValues)
  filterValuesRef.current = filterValues
  // Monotonic guard so two rapid immediate selections can't let a stale
  // (out-of-order) cascade response commit last.
  const immediateSeqRef = useRef(0)
  ...
  /** Immediate apply (apply_behavior='immediate', e.g. a header search box):
   *  change-and-apply in ONE commit, derived from the APPLIED base — NOT
   *  pending — so it never commits another filter's half-edited manual value.
   *  Cascades the change (refreshing dependent options) and folds the resets
   *  into the same commit → one URL push, one widget refetch wave. */
  const applyImmediate = useCallback(async (patch) => {
    if (!patch || typeof patch !== 'object') return
    // Is every patched value already the APPLIED value? (Re-selecting the same
    // patient.) If so, and the cascade produces no resets, we must NOT mint a
    // new filterValues object — that reference change alone refetches every
    // widget. Mirrors WidgetGrid.onCrossFilter's same-as-applied no-op.
    const base = filterValuesRef.current
    const isNoOp = Object.entries(patch).every(([k, v]) => (base[k] ?? '') === (v ?? ''))
    const seq = ++immediateSeqRef.current
    setPendingValues(prev => ({ ...prev, ...patch }))
    let extra = {}
    const resolve = crossFilterResolverRef.current
    if (resolve) {
      try {
        extra = (await resolve(patch, filterValuesRef.current)) || {}
      } catch (err) {
        console.warn('[IMMEDIATE] resolver failed, committing patch only:', err)
        extra = {}
      }
    }
    // A newer immediate selection superseded this one → drop the stale commit.
    if (seq !== immediateSeqRef.current) return
    if (isNoOp && Object.keys(extra).length === 0) return   // nothing changed
    setPendingValues(prev => ({ ...prev, ...patch, ...extra }))
    // prev = current APPLIED values → base + patch + resets, without pending edits.
    setFilterValues(prev => ({ ...prev, ...patch, ...extra }))
  }, [])
