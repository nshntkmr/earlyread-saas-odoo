 ...
 */
// KPI-like types that opt into the compact page-summary six-card breakpoints.
...
export default function WidgetGrid({ initialWidgets, placement = 'tab-content' }) {
  ...
  // ── Page-level placement (render_region='page_summary') ──────────────────
  const isPageSummary = placement === 'page-summary'
  // Shared visibility predicate — a page-summary grid is always visible and
  // never tab-gated; a tab-content grid keeps the original tab_key logic.
  const visibleForGrid = (w) =>
    isPageSummary || !currentTabKey || !w.tab_key || w.tab_key === currentTabKey
  // A settled-empty record_header (no selected record) is filtered out so it
  // leaves no blank card; loading/error states keep their wrapper.
  const isTrulyEmptyHeader = (w) =>
  ...
  useEffect(() => {
    ...
    const visibleWidgets = Object.values(widgetData).filter(visibleForGrid)

    // Mark non-visible (other-tab) widgets as deferred so they re-fetch
    // with new filter values when the user switches to that tab.
    // Page-summary widgets are tab-independent → never deferred.
    if (!isPageSummary) {
      setWidgetData(prev => {
        const updated = { ...prev }
  ...
  useEffect(() => {
    // Page-summary widgets are tab-independent — fully insulate them from any
    // tab-change side effect (drill reset, lazy-load).
    if (isPageSummary) return
  ...
  // ── Visible widgets for this grid (tab-content vs page-summary) ───────────
  const visibleWidgets = Object.values(widgetData)
    .filter(visibleForGrid)
    .sort((a, b) => (a.sequence || 0) - (b.sequence || 0))
  // Drop settled-empty record headers so they leave no blank card.
  const renderableWidgets = visibleWidgets.filter(w => !isTrulyEmptyHeader(w))

  if (!renderableWidgets.length) {
    // Page-summary: render nothing (no wrapper/padding above the tabs).
    if (isPageSummary) return null
    return (
  ...
  const renderWidget = (w) => {
    ...
    // record_header is a hero card — suppress ALL standard card chrome (header,
    ...
    // Compact page-summary KPI (≈17% → 4 of 24 cols) opts into the responsive
    // 6/3/2/1 breakpoints; a deliberately wider KPI keeps its configured width.
    const isCompactSummaryKpi = isPageSummary && KPI_LIKE_TYPES.has(w.chart_type) && gridColSpan === 4

    return (
      <div
        key={w.id}
        className={`pv-widget-col${isCompactSummaryKpi ? ' pv-widget-col--kpi' : ''}`}
        style={{
  ...
  return (
    <div className="pv-content">
      ...
      <div className={`pv-widget-grid${isPageSummary ? ' pv-widget-grid--page-summary' : ''}`}>
        {renderableWidgets.map(w => renderWidget(w))}
      </div>
