...
// MAIN MAP WIDGET
// ═════════════════════════════════════════════════════════════════════════════
export default function MapLibreMap({ data, height, name, onCrossFilter }) {
  ...
  // Choropleth region click action: 'drill' (SVG-Albers only) | 'cross_filter' | 'popup'.
  // MapLibre honors cross_filter + popup; 'drill' has no MapLibre equivalent → popup.
        ...
        <div className="flex-1 relative">
          <Map ref={mapRef} mapStyle={MAP_STYLES[mapStyle]?.url || MAP_STYLES.streets.url}
            ...
            onClick={e=>{
              ...
              const props=data.choropleth_popup_data?.[regionKey]||f.properties
              // Cross-filter: apply a page filter from the clicked region (config-driven).
              // Value = configured column when present; region join key ONLY when no
              // column is configured. Configured-but-absent (no-data region, or props
              // fell back to raw geometry properties) → no-op rather than sending the
              // join key into a param it doesn't belong to.
              if(clickAction==='cross_filter'&&crossFilterParam&&typeof onCrossFilter==='function'){
                const value=crossFilterCol?(props[crossFilterCol]!=null?props[crossFilterCol]:null):regionKey
                if(value!=null) onCrossFilter(crossFilterParam,value)
                return
              }
              setPopupInfo({coords:[e.lngLat.lng,e.lngLat.lat],properties:props})
            }}>
