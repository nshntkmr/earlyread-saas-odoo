...
def register_tools(mcp):
    ...
    async def query_data(source_id: int, sql: str, limit: int = 200) -> dict:
        ...
        rows back. SELECT/WITH only — the server validates, enforces tenant
        scoping and row caps (max 500 rows), and logs every query. If the
        query fails, read the error message, fix the SQL, and try again.
        Always call get_schema(source_id) first."""
        try:
    ...
    @mcp.tool
    async def ask_data(source_id: int, question: str) -> dict:
        """Fallback: ask a natural-language question and let the Posterra
        server's own SQL generator author and run the query. Prefer writing
        SQL yourself with query_data — this tool exists for cases where the
        server's generator handles a dialect better. May be unavailable
        (HTTP 501) if the server has no LLM configured."""
        try:
            return await gateway(
                "POST", "/api/v1/ai/query",
