 ...
 * Uses the designer API to fetch drill-down data.
 *
 * Props:
 *   widgetId       — number (definition id)
 *   clickColumn    — string (the column axis the user clicked)
 *   clickValue     — string (the value the user clicked)
 *   filterParams   — object (current filter params to pass through)
 *   onClose        — () => void
 *   apiBase        — string
 */
export default function DrillDownModal({
  ...
  useEffect(() => {
    ...
    const url = `${apiBase}/library/${widgetId}/drill?${params}`
    setLoading(true)
    setError(null)
  ...
  }, [widgetId, clickColumn, clickValue, filterParams, apiBase])
