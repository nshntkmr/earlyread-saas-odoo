...
- **⚠ v5 D-family quota is 0 SUBSCRIPTION-WIDE** — `DASv5`, `DSv5`, `DDSv5`, `Dv5` etc. sit at a quota *limit* of 0 in **eastus2, eastus AND centralus** (capacity-constrained; Microsoft denied a quota-increase request citing high demand). Confirmed from the Quotas blade — this is **not** an eastus2-only problem, so **changing region does not solve it**; the SKU family must change regardless of where you deploy. Current picture:

  | Family | eastus2 | eastus | centralus |
  |---|---|---|---|
  | DASv4 | 8/10 (dev) → 2 free | 0/10 | 0/10 |
  | DASv5 | **0/0** ❌ | **0/0** ❌ | **0/0** ❌ |
  | Dasv7 | 0/10 → 10 free | 0/10 | 0/10 |

  **Dev** runs `D4as_v4` + `D2s_v4` and is **frozen — do not touch it** (it holds 8/10 of eastus2 DASv4). **Staging is not deployed** (see below), so its v5 config is moot. **Prod** uses the `Dasv7` family. Prod needs ~16-20 vCPU (AKS rule: ≥2 nodes × ≥4 vCPU system pool, plus a user pool), so eastus2's 10 free Dasv7 vCPUs require a **quota increase** — a far likelier approval than v5, since Dasv7 already has a non-zero limit at 0% usage. `eastus`/`centralus` have 20 vCPU free today (Dasv7 10 + DASv4 10) if you'd rather not wait on an approval. Check available families + capacity with: `az vm list-skus --location <region> --resource-type virtualMachines --all` (look for empty `restrictions` — quota ≠ capacity, which is exactly how v5 bit us). Family quota usage is in the Quotas blade or a `QuotaUsage` CSV export — verify all three gates: system-pool family, user-pool family, and Total Regional vCPUs.
