...
export default function FilterBar({ headerBadges = [] }) {
  const {
  ...
  const resolveCrossFilterPatch = useCallback(async (patch, baseOverride) => {
    const valuePatch = {}
    for (const [param, value] of Object.entries(patch || {})) {
      ...
      // Immediate-apply passes the APPLIED base; cross-filter clicks pass none
      // → the pending snapshot (existing behavior, unchanged).
      const base = { ...(baseOverride || pendingRef.current), ...patch, ...valuePatch }
  ...
  // ── Placement split (D1) ───────────────────────────────────────────────
  // Filter Bar filters render in the bar; page_header_* filters portal into
  // the QWeb header slots alongside badges (unified renderer below).
  const regionOf = f => f.display_region || 'filter_bar'
  const visibleFilters = filters.filter(f => f.is_visible !== false)
  ...
  // an Apply button. Both cascade dependent options identically.
  const onChangeFor = (filter) => (val) => {
    const key = filter.param_name || filter.field_name
    if (filter.apply_behavior === 'immediate') {
      applyImmediate({ [key]: val })
    } else {
  ...
  const renderControl = (filter) => {
    ...
    return (
      <FilterControl
        filter={filter}
        options={options}
        value={pendingValues[key] || ''}
        onChange={onChangeFor(filter)}
      />
    )
  }

  // ── Filter bar (only filter_bar filters) ───────────────────────────────
  const bar = barFilters.length > 0 ? (
    <div className="pv-ctx-filter-bar">
      ...
      <div className="pv-ctx-filter-group pv-ctx-apply-group">
        <button type="button" className="btn btn-primary pv-ctx-apply-btn"
                id="ctx-apply-btn" onClick={applyFilters}>Apply</button>
        <button type="button" className="btn btn-outline-secondary pv-ctx-clear-btn"
                id="ctx-clear-btn" onClick={handleClearAll}>Clear All</button>
      </div>
    </div>
  ) : null

  // ── Unified header renderer (filters + badges, one owner) ───────────────
  // Merges header filters + page-header badges into typed items and sorts each
  // slot by (sequence, type, id) so placement is deterministic. Badge-only
  // pages still render their badges (this replaces the old <HeaderActions>).
  const buildSlotItems = (slotRegion, withApply) => {
    const items = []
    for (const f of headerFilters) {
      if (regionOf(f) === slotRegion) {
        items.push({ type: 'filter', seq: f.sequence || 0, id: f.id, filter: f })
      }
    }
    for (const b of (headerBadges || [])) {
      if (b.placement === slotRegion) {
        items.push({ type: 'badge', seq: b.sequence || 0, id: b.id, badge: b })
      }
    }
    items.sort((a, b) =>
      (a.seq - b.seq)
    ...
    const nodes = items.map(i => (
      i.type === 'filter'
        ? <div key={`f${i.id}`} className="pv-header-filter" title={i.filter.name}>{renderControl(i.filter)}</div>
        : <PageBadge key={`b${i.id}`} badge={i.badge} />
    ))
    if (withApply) {
      nodes.push(
        <div key="hdr-apply" className="pv-header-apply">
  ...
  const headerHasManual = headerFilters.some(f => f.apply_behavior !== 'immediate')
  const endHasFilter = headerFilters.some(f => regionOf(f) === 'page_header_end')
