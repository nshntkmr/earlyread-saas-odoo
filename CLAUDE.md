...
- **CH connections require cluster-side bootstrap DDL + a server-config prereq** — the addon's executor depends on:
  1. **Cluster server config** declaring `<custom_settings_prefixes>app_</custom_settings_prefixes>` (in `users.xml` or a `config.d/*.xml` drop-in, then `SYSTEM RELOAD CONFIG;`). Without this, every `SQL_tenant_id` setting the addon sends is rejected with "Setting `SQL_tenant_id` is neither a built-in setting nor started with the prefix listed in custom_settings_prefixes." This is a **server-level** setting only — declaring it in a profile silently has no effect.
