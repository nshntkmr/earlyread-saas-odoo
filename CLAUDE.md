...
- **What it is**: page-level "Export PDF" (`dashboard.page.pdf_export_enabled`, off by default) that prints the CURRENT tab's `record_header` / `kpi` / `status_kpi` / `table` widgets with the applied filters, branding and an optional user Keynote. Charts and other types are listed in the PDF as "not included"; **Snowflake-backed widgets are skipped with a notice and never queried** (v1 = PostgreSQL + ClickHouse only). Route `POST /api/v1/page/<id>/export/pdf` (`controllers/pdf_export_api.py`, `readonly=True`), services in `services/pdf_export/` (collector → document/QWeb `views/pdf_export_templates.xml` → `render_client` → Gotenberg).
