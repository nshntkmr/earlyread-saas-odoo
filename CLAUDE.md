...
- **Per-option ("Different SQL Per Option") tables keep their column config on the SCOPE OPTION, not the widget** — the Designer routes `table_column_config`/x/y to the active option, so the widget-level fields stay empty by design. Any data request WITHOUT `_scope_option_id` must therefore resolve the default option (`dashboard.widget._default_query_scope_option()`: `scope_default_value` match, else first active by sequence; empty recordset when not query mode / no option / no SQL → unchanged widget-level fallback). Used by `portal.app_dashboard` first paint (inline), `widget_api` `/data` and `/download`. React (`WidgetGrid`) seeds `scopeOptionIds` synchronously from the initial widgets so the deferred-tab lazy load on mount (URL `?tab=` → server renders the first tab, React switches) already carries the id. Symptom when any path skips this: one auto-generated header per SQL column on first load, correct columns after the first toggle click.
- **Comparison KPI card has an opt-in `compact_split` layout that shares the classic data contract** — `visual_config.comparison_layout` (`classic` default | `compact_split`) plus `comparison_badge_position` (`title_row`|`beside_value`|`below`; the compact head row is omitted when it would be empty), `comparison_show_side_labels`, `comparison_left_bg/_color`, `comparison_right_bg/_color`. The portal card (`KpiCardGeneric.jsx`) reads them from `data.visual_config`, which `_build_kpi_data` already forwards for every KPI variant — NO server serializer change, so widgets without the keys are byte-identical. Three places must stay in sync: `KpiStylePicker.jsx` (Comparison Settings), `KpiCardGeneric.jsx` (render), and the Designer preview pair `preview_formatter._format_kpi_preview` (emits `prior_formatted`/`current_label`/`prior_label`/`diff_*` for `kpi_style=='comparison'`) + `LivePreview.jsx` (`isComparisonKpi` branch). Both layouts consume `value` (X) / `prior_value` (Y1) / optional `current_label`, `prior_label` SQL columns; the diff badge is always left minus right, and its text honours `comparison_show_absolute` / `comparison_show_pct` (both default True; percent-format cards show the absolute part as `pts`; both off → no badge) via `_comparison_annotation` in `dashboard_widget.py` mirrored by `preview_formatter.comparison_annotation`. A SQL NULL on EITHER side (`value` or `prior_value`) means "no comparison": the tile prints `--` and NO badge / `diff_status` is emitted (a diff against zero would show "+14.5 pts" beside a `--` goal); a zero is still a real value and keeps its badge. Both branches (`_build_kpi_data` comparison + `preview_formatter._format_kpi_preview`) raise past the diff block on `None`.
- **`expandableCount` cell renderer = number + "+" opening a READ-ONLY popover** (`@posterra/grid-utils` `renderers.jsx`, settings in `TableColumnSettings.jsx`). Items come from a JSON column on the SAME row (`cellRendererParams.itemsColumn`; shape `[{label, status, note?}]`, also accepts a `{label: status}` object) so no extra request. Status chip colors: `colors` map (any status text) over six built-in defaults (True Care Gap, Data Gap, Projected, Compliant, Matched, Missed); unknown → gray. The button attaches NATIVE `mousedown`/`click` listeners that `stopPropagation()` — AG Grid's `onCellClicked` is a native cell listener that fires before React's root handler, so React `stopPropagation` would still open the row drawer. One popover at a time (module latch), closes on outside click / Esc / scroll / resize, portalled to `<body>` with `position: fixed`. `@posterra/grid-utils` is a `file:` dependency that npm COPIES into each app's `node_modules` (no junction on this Windows box) and those two copies are TRACKED in git (`.../react/node_modules/@posterra/grid-utils`, `.../designer/node_modules/@posterra/grid-utils`): after editing `shared/grid-utils`, run `npm install` in BOTH `posterra_portal/static/src/react` and `dashboard_builder/static/src/designer` (or the copies stay stale), `npm run build` both, and commit the copies together with the source and the dist bundles.
- **Annotation SQL (`annotation_query_sql`) supports `[[ ... ]]` optional clauses and missing-param NULL binding** — `_execute_annotation_sql` runs `resolve_optional_clauses` and `setdefault(param, None)` exactly like `_execute_sql` (it did NOT before: an annotation query with `[[ AND col IN %(PLAN_NAME)s ]]` failed with a syntax error, was swallowed by the `try/except` in `_interpolate_annotations`, and every subtitle/footnote/badge/tooltip printed its raw `%(...)s` template — that symptom means "the annotation query failed", check the warning log). `{where_clause}` is still NOT supported there (the main query pops `_where_sql` first) — write the filters explicitly. React side: `portal.py` pops `_resolved_subtitle/_footnote/_info_text/_annotation_text` into the widget meta ONLY on first paint; every later fetch (Apply, scope toggle, and the deferred-tab lazy load that fires on mount for `?tab=` URLs) returns them INSIDE `data`, so `WidgetGrid.jsx` must render `w.data?._resolved_X || w.X` for all four (subtitle and footnote were missing that fallback → a lazily loaded tab showed the raw template while the badge resolved). The PDF prints no widget subtitle/footnote at all (only the record-header subtitle), so nothing to mirror there. `subtitle_color` / `footnote_color` (instance-only `dashboard.widget` Char fields, Annotations tab → Labels, page-template round-trip) are validated to a hex literal by `portal._safe_hex` (`''` otherwise) and applied as inline `color`; the grid DROPS Bootstrap's `text-muted` class when a colour is set because that utility is `!important` and would win over the inline style.
- **Header badge (`annotation_type='badge'`) is SQL-driven on per-option tables too, and colourable** — `dashboard_widget_scope_option.execute_option_sql` (table branch only) merges `widget._interpolate_annotations(cols, rows, portal_ctx)` into its result, so `annotation_text` templates like `Evidence uploaded: %(evidence_count)s` (columns from `annotation_query_sql`, else the widget's own first row) resolve for "Different SQL Per Option" tables the same way `get_portal_data` already did for widget-level SQL. `WidgetGrid.jsx` reads `data._resolved_annotation_text || annotation_text` (same refresh pattern as the info tooltip; first paint pops the resolved key into `annotation_text` in `portal.py`). Colour: `portal.py` sends `annotation_color` ONLY when it differs from the field default `#6b7280` (sent as `''` otherwise), and React keeps the classic `badge bg-light text-dark` classes when it is blank — existing badges are byte-identical. A chosen colour becomes the badge text colour with a 12% tint background (`.pv-widget-badge--colored`). The admin field moved from the table-hidden Positioning group into its own "Color" group (`widget_views.xml`) so tables can set it.
- **Table "Total row" pins matching SQL rows to the bottom in bold** — `visual_config.tableTotalRowColumn` + `tableTotalRowValue` (Designer: Table Display block in `TableConfigurator.jsx`, stored through the normal `visualFlags` → `visual_config` path, so nothing new is threaded through create/update/place/library sync). A row is a total row when `String(row[column]) === String(value)` on the RAW row value (hidden columns count; never rendered text). Three mirrors, keep in sync: `DataTable.jsx` (`splitTotalRows` → `pinnedBottomRowData` + `getRowStyle` bold/top rule; "Showing N of M" excludes the pinned rows), the Designer `PreviewGrid` in `TableConfigurator.jsx` (same split, same style), and the PDF `collector.split_total_rows` → `block.table.total_rows` printed as `tr.total` at the END of `tbody` (deliberately NOT `tfoot` — Chromium repeats a tfoot on every printed page of a long table). Blank setting → every path is byte-identical (`total_rows: []`). Pinned rows are outside AG Grid's sort/filter/pagination, which is the point. `document.build_view` must COPY `total_rows` from the collector block into `view['table']` (the template only sees the view — forgetting this silently drops the row from the PDF while the grid still pins it). Also in `build_view`: `pdf_page_break_before` on the FIRST printed widget is ignored — honouring it left page 1 holding only the title block.
- **Record Header has an opt-in `scorecard` layout** (`visual_config.header_layout` = `classic` default | `scorecard`) — the shared `dashboard_builder/services/record_header_formatter.py` (portal + Designer preview, single source of truth) adds `subtitle` (`subtitle_column`), `stats` (`stats: [{column, label, icon (Font Awesome class), format number|percent|text, icon_color, label_color, value_color, bg, prior_column, higher_is_better}]`, thousands separators, up to 6; header-wide colour defaults `stat_icon_color/_label_color/_value_color`, `stat_bg` with per-tile overrides, blank keys omitted from the payload; `prior_column` → `trend {dir up|down|flat, status good|bad|neutral, delta}` with percent deltas in pts) and `chips` (`chips_column`: CSV or JSON array, `chips_label`, `chip_color`, `chip_text_color`) plus `avatar_shape`; every configured-but-missing column fails closed with the same `{'error': ...}` shape as the classic footer. Classic payload is byte-identical when the key is absent (locked by `dashboard_builder/tests/test_record_header_formatter.py`, runs standalone with plain python). Designer: `RecordHeaderConfig.jsx` is the "Configure Header" step body for `record_header` (Custom SQL only — columns come from the SQL test result), writes to `visualFlags` (`SET_VISUAL_FLAG`) so nothing new is threaded through create/update/place/library sync; it also exposes classic `label_overrides`, which the formatter always supported but no Designer field set before. Renderers: `RecordHeader.jsx` (portal, `.pv-record-header--scorecard`) and `RecordHeaderPreview.jsx` (Designer, inline styles) — keep them visually in sync.
...
### Part D — `remote_autocomplete` filter + header placement (Find-patient search)
- **`remote_autocomplete` NEVER preloads its roster** — that is the entire point (patient/EID rosters are 30-50K rows). `dashboard_page_filter.get_options()` returns `[]` at the TOP for `ui_type=='remote_autocomplete'` — this central guard protects EVERY caller (both page-config paths, cascade, resolve, the legacy `/posterra/filter_options` route). Only `search_options_page()` + `hydrate_options()` may return remote options. Both page-config builders (`portal.py` filter loop + `widget_api.api_page_config`) ALSO skip `get_options()` for remote and serialize `options: []`. Adding a new caller that enumerates the roster reintroduces the multi-second preload — always go through the paged search endpoint.
- **Search applies `current_values` (sibling constraints); hydration must NOT** — `search_options_page` ANDs the other applied filter values into the WHERE (search within the selected provider). `hydrate_options` (resolving a known value's label for deep-link/Back-Forward) is IDENTITY resolution scoped ONLY by server-derived `provider_ids` — sending sibling constraints would blank the label of a legitimately-selected value that doesn't match a sibling. So `RemoteAutocomplete.jsx` sends `current_values` on the SEARCH shape but `{values:[v]}` (no current_values) on the HYDRATION shape. The security boundary is `provider_ids`, never `current_values` (a forger just omits them).
- **search/hydrate `constraints` are RECORD-keyed** — they pass `constraints` straight to `_build_schema_where`, which keys on the filter RECORD, UNLIKE `get_options`' int-keyed `constraint_values`. Both callsites build record-keyed dicts: `widget_api.api_filter_search` and the remote-target branch in `api_filters_resolve`. An int-keyed dict → `AttributeError` on `src_filter.model_name`.
- **Remote SQL fails CLOSED on scope + uses a TRIM-normalized value expr** — when `scope_to_user_hha` is on, `search_options_page`/`hydrate_options` return empty unless `_build_schema_where` actually emitted its tenant predicate (`'hha_ccn_list' in params`) — an unset/invalid `hha_scope_column_id` would otherwise fail OPEN (a config constraint also requires the scope column for remote filters). Dedup/blank-check/hydration-equality all use `_remote_value_expr` (`TRIM(CAST(col AS TEXT))` / `trimBoth(toString(col))`) IDENTICALLY — partitioning on the raw value while emitting `str().strip()` lets whitespace-variant keys break pagination + miss hydration. Search endpoint is `POST /api/v1/filters/<id>/search` (term must not enter URLs), `no-store` + `Vary: Authorization` on every response, raw query never logged, `503` (sanitized) on executor error. `models.ValidationError` does NOT exist in Odoo 19 — import `from odoo.exceptions import ValidationError`.
- **`display_region` places any filter in the page header; FilterBar owns ONE unified header renderer** — filters split into bar (`filter_bar`/blank) vs header (`page_header_start/end`); the header renderer (in `FilterBar.jsx`, NOT the deleted `HeaderActions`) merges header filters + page-header badges and sorts each slot by `(sequence, type, id)`, portalling into `#pv-page-header-actions-start/-end`. Do NOT re-add a standalone `<HeaderActions>` mount — badges would double-render. `App.jsx` passes `headerBadges` to `<FilterBar>`. Server computes `has_page_header_start/end` (badges OR visible filters) to gate the QWeb slots. The compact header Apply/Clear renders EXACTLY ONCE (not per slot).
- **Page header look is per-page config: subtitle style + a badge strip under the subtitle** — `dashboard.page.subtitle_style` (`plain` default = the untouched `<small class="text-muted">`, `accent_bar` = `<small class="pv-page-subtitle--accent">` with optional `subtitle_color` / `subtitle_accent_color`, validated by `portal._safe_hex`) and badge placement `page_header_strip` ("Page Header — Under subtitle"), rendered by the SAME FilterBar header renderer into `#pv-page-header-strip` (no Apply button there — filters never target the strip). The server adds `pv-page-header--with-strip` to the header when any active badge uses that placement, which switches the grid to a third `strip` row; without it the two-row template is byte-identical. `dashboard.page.badge.badge_style` (`text` default | `pill`): `PageBadge.jsx` adds `pv-page-badge--pill` (+ `--tinted` when a Text Color is set → `color-mix` 12% tint of `currentColor`). Header badges are initial-paint only (no React refresh path), same as the start/end slots. Page templates now export/restore badge `placement` (absent → NULL legacy slot) and `badge_style`, plus the three page subtitle fields.
- **`apply_behavior='immediate'` applies from the APPLIED base with no-op + seq guards** — `FilterContext.applyImmediate(patch)` derives the commit from `filterValuesRef.current` (applied), NOT pending, so it can't commit another filter's half-edited manual value. It SKIPS the commit when the patch equals the applied value and the cascade yields no resets (a new `filterValues` reference alone refetches every widget — the same no-op invariant as `WidgetGrid.onCrossFilter`), and a monotonic `immediateSeqRef` drops a stale out-of-order cascade response. `RemoteAutocomplete` hydrates the label on every value change (mount/select/Back-Forward), resetting `hydratedForRef` on a transient error so a later trigger retries.
- **Widget-level filters (`dashboard.widget.filter`) are ADDITIVE to the single scope control** — a widget can carry N controls (dropdown / multi-select / toggle) in its own header, bound ONLY into that widget's SQL via the usual `[[ AND col = %(param)s ]]`. Three `options_mode`s: `static` (value|Label lines), `schema` (DISTINCT of a schema-source column through the executor), `mirror` (borrows a PAGE filter's options + param; the widget value OVERRIDES the page value for this widget only, blank INHERITS). Runtime: React sends `_wf_<param>` on every fetch of that widget (`WidgetGrid.wfParamsFor` — Apply, scope, drill, lazy load, download, detail); `widget_api._apply_widget_filters` reads ONLY params declared on `widget_filter_ids` (whitelist — nothing can be smuggled through the prefix) and `utils/widget_filters.apply_widget_filter_values` binds them into a COPY of `portal_ctx` (portal.py shares one ctx per tab — never mutate). Initial paint binds the filters' `default_value`s server-side (`portal.py._ctx_for_widget_with_filters`), the same first-paint trap the scope options had. Guard rails (model constraints, both directions): a static/schema filter may NOT reuse a page filter's runtime key (mirror is the sanctioned override) and a page filter may not take a widget filter's key; a mirror may NOT target a provider-selector / HHA-scoped / auto-filled / hidden page filter (tenant scoping stays intact); two filters on one widget cannot share a key. Widgets with no records are byte-identical (helper returns the same ctx object; payload key `widget_filters` present only when configured). Threading checklist (mirror of the scope-option one): portal payload, 3 widget_api endpoints, template export/restore (`sync_for_widgets(..., strict=True)`), designer_api detail/create/update/place (+ `_widget_filters_stash`), builder_api create/update/detail/place, `WidgetControls` (renders via `FilterControl`, so widget filters look like page filters), `WidgetFiltersSection` in the Designer's Controls step, `CustomSqlEditor.extraParams` pills. Not covered: ORM-mode widgets (no SQL), page sections/badges (not widgets), `{where_clause}` auto-binding (explicit `[[ ]]` only), URL/permalink persistence (React state only).
- **Page-template restore is preflight-first + connector-identity** (`dashboard_page_template.py`) — `_preflight_filters` resolves every filter's Schema Source, value/search/scope columns, Display-Template placeholders, and Remote Search invariants BEFORE any record is created; any unresolved reference raises `ValidationError` before `Page.create` (a failed restore leaves NO page/tab/filter). `_resolve_template_source` binds by portable identity `{table_name, engine, connection_name}` (explicit `__local_pg__` marker for NULL `connection_id`); a legacy table-only template resolves ONLY when exactly one source matches, else raises (never an arbitrary wrong-tenant bind, never a silent downgrade to a preloading dropdown).

### Part F — Projections ("Mark Projected Compliant", Sept 2026)
- **What it is**: a saved *expectation* per identity (e.g. `HUM_ID + MEASURE_CAT + MEASUREMENT_YEAR`) that the next monthly data refresh will show the measure compliant. Config-driven: a **Projection Type** (`dashboard.projection.config`, Posterra → Configuration → Projection Types) per app names the source Schema Source, identity columns, snapshot (month) column, compliant/eligible values, permissions, popover fields, labels/colours. Nothing changes for any app until a type exists AND a drawer section carries the block.
- **Drawer block lives in the existing Detail Drawer JSON** (`detail_drawer_config.sections[i].card.projection`: `type_key`, `identity` aliases, `snapshot_alias`, `member_key_column`, `month_filter_param`). Authored on the **definition** (Advanced tab) and carried to instances by the normal `library_update` sync. NOTE: the only constraint on `detail_drawer_config` is `_check_tab_scoped_placeholders` (rejects SQL referencing a filter scoped to another tab) — there is NO JSON-shape or projection-block validation. Malformed JSON is swallowed by `_get_detail_drawer_config()` (warning logged, returns `{}`, drawer silently stops opening); a bad projection block surfaces at runtime as `projection_meta.error` on the section. Validate the JSON before saving. Also: the field is exposed ONLY on the `dashboard.widget` form (Advanced tab), not on the definition form, and `_onchange_definition` copies `defn.detail_drawer_config` over the instance — re-picking a library definition wipes a drawer authored only on the instance.
- **One record per cycle** (`dashboard.projection`, unique `(config, identity_hash, cycle_no)`), never deleted; `dashboard.projection.event` is append-only (only outbox/`result_json` fields writable). `revision` is identity-wide (max over cycles); every mutation sends `expected_revision` → 409 `stale_revision` names the other actor. Mark sends no `cycle_no`; Edit/Re-project/Undo target the cycle covering the viewed month (409 `cycle_mismatch` otherwise). Mark/Re-project only on the identity's latest available month unless `allow_historical_mark`.
- **Outcome is computed at read time, never stored** (`utils/projection_outcomes.py` = the same rule as the rendered reporting view): per month → `''` / `projected` / `missed` / `matched` / `ineligible`; the match month is the earliest eligible compliant month a cycle covers; months after it show nothing; Undo cancels from `undone_month` (earlier months keep their status); admin Void sets `undone_month = F`. Months compare as text — snapshot values must sort chronologically (`YYYYMM`).
- **Identity hash must be identical in Python/PG/CH** (`utils/projection_identity.py`): trim `' \t\r\n'`, `<utf8_len>:<value>` per component, MD5 lowercase hex; incomplete → NULL/'' (un-projectable). The type form renders the expressions, the mirror DDL and the whole reporting-view SQL; **the admin creates the ClickHouse objects by hand** (Odoo never runs DDL on CH) and registers the view as a Schema Source with `projection_type_id` set so widgets on it are consumers.
- **Service path** (`services/projection_service.py`): re-runs the drawer section SQL through `_run_detail_query` (same scoped executor), keeps the row whose hash AND snapshot match, runs the member history query on the source, `pg_advisory_xact_lock` per identity, idempotent `request_id` (fingerprint mismatch → 409), residual unique violation → `ConcurrencyError` (Odoo retries). Endpoints: `POST /api/v1/projections/{mark,edit,reproject,undo}` (`readonly=False`; body carries the applied `filters`).
- **Overlay** (`services/projection_overlay.py`, called from `_execute_drawer_detail(..., user, providers)`): rows get `__pv_identity_hash/_month/_source_positive/_cycle_no/_outcome` and `__pv_can_mark/_edit/_reproject/_undo`; **buttons render only from these flags, never from the colour**. Sections without the block are byte-identical. `projection_meta.can_act` is false without a user (read-only).
- **Publisher** (`utils/projection_publisher.py`): `dashboard.connection.purpose='publisher'` (INSERT-only CH user) — the executor factory, Designer dropdown and schema-source picker refuse such connections. The ONLY exemption is the connection's own Test Connection, which passes `allow_publisher=True` and just pings (`SELECT 1`, no table access); every data-reading caller, including the PHI source validation that uses `allow_inactive=True`, must keep refusing. Post-commit hook + 1-min cron publish `pending/failed` events to `mirror_table` (`ReplacingMergeTree(revision)` keyed with `cycle_no`), `dead` after 20 attempts (admin Republish); daily `check_mirror` re-queues cycles whose latest revision is missing. PG-backed types publish nothing (`publish_state='none'`).
- **React**: `ProjectionControls.jsx` (chip/buttons/popover/history), `DetailDrawer.jsx` (silent refetch after save + `poll_seconds` timer, drafts survive refreshes, 409 keeps the draft, "Newer data available — Show" via `applyImmediate`), `WidgetGrid.refreshProjectionConsumers()` (refetch `projection_consumer` widgets with no skeleton). Rebuild with `npm run build`.
- **Tests**: `--test-tags posterra_projections` (identity fixtures incl. PG expression equality, outcome timelines, service flows, publisher with a fake client, drawer overlay). Never run them against the live DB — use a copy (`odoo_dev_proj`, launch config `odoo-copy-projections`).

### Part G — Page PDF export (Sept 2026; plan `~/.claude/plans/page-pdf-export-playwright-commentary.md` v5)
- **What it is**: page-level "Export PDF" (`dashboard.page.pdf_export_enabled`, off by default) that prints the CURRENT tab's `record_header` / `kpi` / `status_kpi` / `table` widgets and standard ECharts charts (`charts.PRINTABLE_CHART_TYPES`: bar, line, pie, donut, radar, scatter, heatmap, sankey, gauge, gauge_kpi; plus KPI sparkline / mini-gauge) with the applied filters, branding and an optional user Keynote. Maps, bullet / RAG / percentile gauge styles and other custom widgets are listed in the PDF as "not included"; **Snowflake-backed widgets are skipped with a notice and never queried** (v1 = PostgreSQL + ClickHouse only). Route `POST /api/v1/page/<id>/export/pdf` (`controllers/pdf_export_api.py`, `readonly=True`), services in `services/pdf_export/` (collector → document/QWeb `views/pdf_export_templates.xml` → `render_client` → Gotenberg).
- **Same data as the screen, never the download path**: the collector runs each widget through `_build_portal_ctx` + `_apply_widget_filters` + the shared `widget_api._resolve_widget_scope` (the `/data` route uses the SAME helper, `strict=False` = its historical fallback; the PDF uses `strict=True` → an invalid explicit option id is a 400) + `execute_option_sql` / `get_portal_data`. Never `download_sql` / `get_download_data`. The collector memoizes the ctx per (tab bundle, params) but COPIES `sql_params` per widget — `/data`'s parameter-mode binding mutates it in place.
- **Opt-in bounded execution**: `executor.execute_bounded(...)` (PG + CH; base class and Snowflake raise `NotImplementedError` — never execute-then-slice). Reached only when the context carries `pv_execution_limits` (read by `dashboard_widget._execute_sql`, `_execute_annotation_sql` (timeout only) and `scope_option.execute_option_sql`); results come back through the mutable `pv_execution_report` context dict. Absent key → the unchanged `execute()` path (byte-compare tested). Tables are bounded by a SQL wrap (`utils/query_executors/bounded.py`: `SELECT * FROM (\n<sql>\n) AS _pv_bounded [ORDER BY …] LIMIT n+1`; trailing `;` stripped outside comments/literals, a newline before `)` neutralises trailing `--` comments, CH `FORMAT` rejected, `%` doubled in PG sort identifiers). Client-side `fetchmany` does NOT bound memory (psycopg2 buffers the whole result) and CH `max_result_rows`+`break` overshoots by a block — don't "simplify" to either.
- **Default sort before truncation**: the AG Grid `sort`/`sortIndex` of the EFFECTIVE columnDefs (option → first sibling with config → widget, mirroring `execute_option_sql`) becomes the wrap's `ORDER BY` (ASC NULLS FIRST / DESC NULLS LAST = AG Grid's null ordering). Unknown sort column (PG 42703 / CH code 47) → one retry without the sort + a PDF notice. KPI/header queries get the timeout only (their formatters read the full result).
- **Timeouts**: `SET LOCAL statement_timeout` survives `RELEASE SAVEPOINT` — `PostgresLocalExecutor.execute_bounded` captures the previous value and restores it in `finally` OUTSIDE the savepoint. CH uses per-query `max_execution_time = min(timeout, connection timeout)`. Budget (`services/pdf_export/deadline.py`, 25 s default under the 30 s App Gateway ceiling): collect = total − render reserve (8 s = Gotenberg `--api-timeout`) − audit reserve; widgets not reached print as "not loaded within the time limit".
- **Locks**: one export cluster-wide + one per user (`pg_try_advisory_xact_lock(hashtext('posterra_pdf_export:global'|':user:<uid>'))` on the read-only request cursor) → 429 with a friendly message.
- **Audit** (`dashboard.pdf.export.log`): autonomous-cursor `started` (fail-closed → 503) + one terminal event (partial unique index on `started_id`); reconciler cron appends `completion_unknown`; retention = private `@api.autovacuum` SQL delete (ORM write/unlink always raise). Stores filter PARAM NAMES + an HMAC fingerprint (`POSTERRA_PDF_FINGERPRINT_KEY`, ICP fallback generated on an autonomous cursor) — never values or keynote text. HttpCase note: the harness refuses a read/write TEST cursor from a read-only one, so full-path route tests set `_registry_readonly_enabled = False`, and a separate read-only test patches the two audit calls to prove the route never writes on the request cursor.
- **Print mirror** (`services/pdf_export/cell_format.py`): renderer wins over valueFormatter; `percentage` formatter + `pctColored` multiply unless `multiply === false`, `barInline` only when `multiply === true`; JS number semantics (`toFixed` ties away from zero, `Number('') === 0`). `cellClassRules` are evaluated by a restricted parser (simple comparisons + `&&`/`||`), never executed. Fixture `tests/fixtures/pdf_cell_format.json` is asserted by Python AND by `node --test posterra_portal/static/src/react/scripts/test_pdf_cell_format_parity.mjs` against the real formatters — update both sides together.
- **Widget PDF fields** (`pdf_include`, `pdf_page_break_before`, `pdf_col_span`) live on `dashboard.widget.action.mixin` (definition + instance); seeded on PLACE, instance-owned afterwards (never in `library_update`'s `sync_vals`), helpers in `dashboard_builder/services/pdf_layout.py`. Page + widget fields round-trip through page templates. `page_config` carries `pdf_export` ONLY when enabled; `portal.py` opens the header END slot for PDF-enabled pages; `ExportPdfButton` renders once in it and reads per-widget scope/filter state from FilterContext's `widgetStateRegistryRef` (filled by each WidgetGrid). The widget form shows these fields on their own "PDF Export" tab — the per-widget Download feature (tab, fields, `/download` route) is deliberately untouched.
- **Per-tab + button colours**: `dashboard.page.pdf_tab_ids` (empty = every tab) is enforced server-side by `pdf_tab_allowed` (route → 403) and mirrored client-side through `pdf_export.tab_restricted` + `tab_keys` — keep the two rules identical (a restricted list whose tabs are all inactive means NO tab, never "all"). Page templates carry the selection as tab KEYS (`pdf_tab_keys`) and fail the restore on an unknown key rather than dropping it. `pdf_button_bg_color` / `pdf_button_text_color` are validated with `cell_format.safe_color` (constraint) and applied as inline styles; a background without a text colour gets white text.
- **Charts** (`services/pdf_export/charts.py`): drawn INSIDE the render service from the SERVER's chart option (`echart_json`, exactly what `EChartWidget` passes to `setOption`), never from browser images. Library = vendored `static/lib/echarts/echarts.min.js` (`ECHARTS_VERSION`, must equal the portal's `node_modules/echarts` version — a test asserts it; bump `VERSION` + the constant when the portal upgrades), loaded as a `data:` URI. Specs are embedded with `embed_json` (`<`, `>`, `&`, U+2028/9 escaped, so data can never close the script). `printable_option` turns animation and progressive rendering off (`progressive: 0` per series) and hides toolbox / slider zoom (range kept), so every chart draws synchronously before the load event — the render call sends NO `waitForExpression`: a wait would time out EVERY export on a renderer without JavaScript. Each chart box starts with a "Chart not drawn" placeholder that the script clears, so a JS-off renderer still returns the PDF. Charts are drawn at explicit pixel sizes computed from the paper width (`document.content_width_px` / `card_inner_width_px` — the 12-col / 7px gap / 18px card chrome constants must match the template CSS). `mini_gauge` KPIs print the portal's split card (ring + text, no CSS bar); `gauge_kpi` also prints its sub-KPI tiles and alert line. Keep `PRINTABLE_CHART_TYPES` in sync with the Designer's `PdfExportOptions.jsx` `PRINTED_TYPES`.
- **Render service**: Gotenberg `8.37.0-chromium` pinned by digest (Helm `pdf.*`, off by default; needs KV secrets `pdf-render-user`, `pdf-render-password`, `pdf-fingerprint-key` before enabling). Basic auth is REQUIRED — the dev AKS cluster runs `networkPolicy=none`, so the chart's NetworkPolicy is not enforced there. Deny-list keeps the pinned default `^file:(?!//\/tmp/).*` plus `^(https?|wss?|ftp)://.*`; private IPs denied, downloadFrom/webhooks off; read-only rootfs + tmpfs `/tmp`/HOME. **JavaScript is ON** (for charts; approved 2026-09-21) — containment was re-verified with JS on: none of 14 sub-request kinds (incl. script `fetch`/XHR/`Image`/WebSocket/`sendBeacon`/`window.open`) reached a probe, and no file outside the request directory loaded. Do not re-add `--chromium-disable-javascript` (charts would print their placeholders). `/health` stays open under basic auth; queue size counts the in-flight conversion; overflow is 429.
- **Helm: PDF env is serving-pod only** (`posterra.pdfEnv`, included by `posterra.podSpec` when `pdf.enabled`; never in `posterra.commonEnv`). The init Job is a `pre-upgrade` hook and runs BEFORE Helm updates the `odoo-secrets` ExternalSecret, so an env var that references a key added in the same upgrade leaves the Job in `CreateContainerConfigError` until the hook times out. Rollout order per env: Key Vault secrets first (Terraform `envs/<env>/main.tf` `initial_secrets`: `pdf-render-user`, `pdf-render-password`, `pdf-fingerprint-key`), then the `helm upgrade` that sets `pdf.enabled` — a missing KV secret fails the whole ExternalSecret sync and the new serving pods cannot start. A minute of `CreateContainerConfigError` on the serving pods right after that upgrade is expected (Helm applies the ExternalSecret last). Roll back with `helm upgrade` to the previous tag and `pdf.enabled: false`, not `helm rollback` (only an upgrade re-runs the module-upgrade Job).
- **Tests**: `--test-tags posterra_pdf_export` (+ the node parity script) on a scratch DB. Fresh installs work again since the Icon Registry menu moved into `dashboard_builder/views/menuitems.xml` (it referenced its parent before it existed).
