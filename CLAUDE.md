...
- **Drawer block lives in the existing Detail Drawer JSON** (`detail_drawer_config.sections[i].card.projection`: `type_key`, `identity` aliases, `snapshot_alias`, `member_key_column`, `month_filter_param`). Authored on the **definition** (Advanced tab; now exposed on the definition form too) and carried to instances by the normal `library_update` sync; validated additively by `_check_detail_drawer_config`.
